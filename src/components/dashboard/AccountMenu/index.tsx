...
export default function AccountMenu() {
  ...
  async function handleLogoutClick() {
    ...
    if (response?.error) {
      toast.error('Une erreur est survenue lors de la deconnexion. Veuillez réessayer.');
    } else {
      router.refresh();

      toast.success('Vous avez bien ete deconnecté.');
  ...
  return (
    <Menu>
      ...
      <Menu.Items className='mt-px w-56 bg-primary text-white'>
        ...
        <Menu.Item className='hover:bg-white/5' href={routes.profile}>
        ...
        <Menu.Divider className='bg-white/[0.08]' />
        <Menu.Item className='hover:bg-white/5' onClick={handleLogoutClick} href={routes.home}>
